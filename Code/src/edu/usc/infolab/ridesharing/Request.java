...
public class Request implements Comparable<Request> {
  ...
  public class AssignmentStat {
    public AssignmentStat() {
      this.failureReasons =
          new HashMap<FailureReason, Integer>() {
            {
    ...
    private AssignmentStat(AssignmentStat other) {
      ...
      this.failureReasons = new HashMap<FailureReason, Integer>(other.failureReasons);
    }
  ...
  public Request(GPSPoint source, GPSPoint dest, Time requestTime, int maxWaitTime) {
    ...
    Pair<Double, Double> shortestPath =
        ShortestPathInMilesAndMinutes(this.source.point, this.destination.point);
  ...
  private Pair<Double, Double> ShortestPathInMilesAndMinutes(GPSPoint s, GPSPoint d) {
    ...
    return new Pair<Double, Double>(
        milesAndMillis.First, milesAndMillis.Second / Time.MillisInMinute);
  }
  ...
  public String PrintShortResults() {
    ...
    results.append(
        String.format(
            ...
            id,
            stats.assigned,
            stats.schedulingTime,
            stats.assignmentTime,
            stats.potentialDrivers,
            stats.acceptableBids,
            source.toString(),
            destination.toString(),
            Time.sdf.format(requestTime.GetTime()),
            maxWaitTime,
            Time.sdf.format(latestPickUpTime.GetTime()),
            optTime,
            optDistance,
            Time.sdf.format(pickUpTime.GetTime()),
            pickUpDistance,
            Time.sdf.format(dropOffTime.GetTime()),
            dropOffDistance,
            detour,
            actualTime,
            actualDistance,
